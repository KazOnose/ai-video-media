...
$s = Get-ChildItem -Path $brain -Filter "ld_knowledge_retention_refined_v2_*.png" | Select-Latest
if ($null -eq $s) {
    # If Select-Latest fails, try sorting and selecting first
    $s = Get-ChildItem -Path $brain -Filter "ld_knowledge_retention_refined_v2_*.png" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
}

CropTo16x9 -inputPath $s.FullName -outputPath (Join-Path $ldDir "knowledge-retention-learning-video.png")
